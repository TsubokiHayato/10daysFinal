...
void Field::Initialize(TuboEngine::Camera* camera) {
	...
	for (int z = 0; z < rows_; ++z) {
		for (int x = 0; x < cols_; ++x) {
			...
			// 格子の中心がワールド原点に来るよう配置
			float px = -halfX_ + step * 0.5f + x * step;
			float pz = -halfZ_ + step * 0.5f + z * step;
			tile->SetPosition({px, 0.0f, pz});
			tile->SetScale({tileScale_, 1.0f, tileScale_});

			// 市松模様：見下ろしでマス目が読めるよう2色を交互に
			if (((x + z) & 1) == 0) {
				tile->SetModelColor({0.32f, 0.36f, 0.42f, 1.0f});
			} else {
				tile->SetModelColor({0.24f, 0.28f, 0.34f, 1.0f});
			}
	...
	// 奥(+Z)・手前(-Z) の壁
	AddWall(camera, {0.0f, wallH, halfZ_}, {0.0f, 0.0f, 0.0f},
	        {halfX_, wallH, 1.0f}, wallColor);
	AddWall(camera, {0.0f, wallH, -halfZ_}, {0.0f, 0.0f, 0.0f},
	        {halfX_, wallH, 1.0f}, wallColor);
	// 右(+X)・左(-X) の壁
	AddWall(camera, {halfX_, wallH, 0.0f}, {0.0f, halfPi, 0.0f},
